...
public class EAO implements GameInterface {
    private long[][] grid;
    private long score;
        ...
        for (int x = 0; x < 4; x++) {
            for (int y = 1; y < 4; y++) {
                ...
                if (tile_val > 0) {
                    ...
                    int[] potential_mege_coord = {x, new_y-1};

                    if (
                        new_y > 0
                        && tile_val == grid[x][new_y-1]
                        && !merged.contains(potential_mege_coord)
                        ) {
                            grid[x][y] = 0;
                            grid[x][new_y-1] *= 2;
                            merged.add(potential_mege_coord);
                            reward += tile_val * 2;
                            moved = true;

                        } else if (new_y != y) {
                            grid[x][y] = 0;
                            grid[x][new_y] = tile_val;
                            moved = true;
                        }
                }
            }
    ...
    public long step(Direction direction) {
        ...
        MoveReturn move_out = move(direction);

        if (move_out.moved()) {
            new_tile();
        }

        score += move_out.reward();

        return move_out.reward();
    }
}
